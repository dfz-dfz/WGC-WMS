...
class DesignofferController extends CommonController {
	...
	public function getList(){
		...
		$nowpage = $page;
		$userid = '';
		$data = M('designoffer')->alias('w')->join('ecm_gongyingshang g ON w.send_uid = g.user_id')->join('ecm_member m ON m.user_id = g.user_id')->field('w.*,g.name,m.user_name')->where($where)->order('w.id desc')->page($page,100)->select();
	    if($page == 0){
			$page = 1;
		}
		/*$where = array();
		...
		if(!empty($keyword) && ($keyword !== 'null')){
			...
			$where['bj_name'] = array(array('like','%'.$keyword.'%'),array('exp','is not null'),'and');
			$where['_logic'] = 'or';
		}
		$data = array();
		$rowcount = 0;
		if(!empty($where)){
			$rowcount = D('DesignofferView')->distinct(true)->where($where)->count();
			//页码计算
			...
			$data = D('DesignofferView')->distinct(true)->where($where)->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('DesignofferView')->where($where)->count();
		}else{
			$rowcount = D('DesignofferView')->count();
			//页码计算
			$pagecount = ceil($rowcount/20);
			if(($page >= $pagecount) && ($pagecount != 0)){
				$page = $pagecount;
			...
			$data = D('DesignofferView')->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('DesignofferView')->count();
		}*/
		//获取对应的项目名称
		foreach($data as $kk=>$xm){
			...
			$data[$kk]['rev_name'] = $rev_name;
		}
		
		$data = array();
		$rowcount = 0;
		 if(!empty($where)){ 
			$rowcount = D('DesignofferView')->where($where)->count();
			//页码计算
			$pagecount = ceil($rowcount/20);
			if(($page >= $pagecount) && ($pagecount != 0)){
				$page = $pagecount;
			...
			$data = D('DesignofferView')->where($where)->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('DesignofferView')->where($where)->count();
		}else{
			$rowcount = D('DesignofferView')->count();
	...
	public function setRevname(){
		...
		if($id && $mobile){
			if($mobile !== 'null'){
				$rev_name = M('designoffer')->where(array('id'=>$id))->getField('rev_name',$rev_name);
				...
				M('designoffer')->where(array('id'=>$id))->setField('rev_name',$rev_name);
				$res = array(
					'code' => 200,
					'message' => '设置成功！'
				);
	...
	public function getDetail(){
		...
		$id = I('get.id',0,'intval');
		$info = D('DesignofferView')->where(array('id'=>$id))->find();
		if(!empty($info)){
			$rev_name = M('designoffer')->where(array('id'=>$id))->getField('rev_name',$rev_name);
			$rev_name = trim($rev_name,',');
			$rev_name = explode(',',$rev_name);
			...
			//回复列表
			$data = D('DesignofferreplyView')->where(array('ds_id'=>$info['id']))->order('id asc')->select();
			//按回话id分类回复
			$hh_idarr = D('DesignofferreplyView')->distinct(true)->where(array('ds_id'=>$info['id']))->getField('hh_id',true);
			$mans = array();
	...
	public function huifu(){
		...
		//$id = 12;
		$info = M('designoffer')->where(array('id'=>$id))->find();
		$res = array();
		//分割多个图像字串
		...
		if(!empty($info)){
			...
			$huifu = D('DesignofferreplyView')->where($where)->order('id asc')->select();
			if(!empty($huifu)){
	...
	public function huifudata(){
		...
		$ids=M('designofferreply')->add($data);
		if($ids){
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/huifu/id/'.$id;
			//Header("Location:{$url}");  die;
			$this->redirect('Designoffer/huifu', array('id' => $weixiu_id ,'userid' => $rev_uid), 1, '回复成功...');die;
		}else{
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/addhuifu/id/'.$id.'/type/'.$type;
			//Header("Location:{$url}");  die;
			$this->redirect('Designoffer/huifu', array('id' => $weixiu_id , 'userid' => $rev_uid), 1,'回复失败...');die;
		}
