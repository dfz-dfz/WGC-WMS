...
class BudgetofferController extends CommonController {
	//预算报价列表
	//设计报价列表
	public function getList(){
		...
		$rowcount = 0;
		if(!empty($where)){
			$rowcount = D('BudgetofferView')->distinct(true)->where($where)->count();
			//页码计算
			...
			$data = D('BudgetofferView')->distinct(true)->where($where)->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('BudgetofferView')->where($where)->count();
		}else{
			$rowcount = D('BudgetofferView')->count();
			//页码计算
			$pagecount = ceil($rowcount/20);
			if(($page >= $pagecount) && ($pagecount != 0)){
				$page = $pagecount;
			...
			$data = D('BudgetofferView')->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('BudgetofferView')->count();
		}
		//获取对应的项目名称
	...
	public function deletes(){
		if(!IS_GET){die;}
		$id = I('get.id');
		$sql = M('budgetoffer')->where(array('id'=>$id))->delete();
		if($sql){
			$this->success( '删除成功!');
		}else{
			$this->error( '删除失败!');
	...
	public function setRevname(){
		...
		if($id && $mobile){
			if($mobile !== 'null'){
				$rev_name = M('budgetoffer')->where(array('id'=>$id))->getField('rev_name',$rev_name);
				...
				M('budgetoffer')->where(array('id'=>$id))->setField('rev_name',$rev_name);
				$res = array(
					'code' => 200,
					'message' => '设置成功！'
				);
	...
	public function getDetail(){
		...
		$id = I('get.id',0,'intval');
		$info = D('BudgetofferView')->where(array('id'=>$id))->find();
		if(!empty($info)){
			$rev_name = M('budgetoffer')->where(array('id'=>$id))->getField('rev_name',$rev_name);
			$rev_name = trim($rev_name,',');
			$rev_name = explode(',',$rev_name);
			...
			//回复列表
			$data = D('BudgetofferreplyView')->where(array('ds_id'=>$info['id']))->order('id asc')->select();
			//按回话id分类回复
			$hh_idarr = D('BudgetofferreplyView')->distinct(true)->where(array('ds_id'=>$info['id']))->getField('hh_id',true);
			$mans = array();
	...
	public function huifu(){
		...
		//$id = 12;
		$info = M('budgetoffer')->where(array('id'=>$id))->find();
		$res = array();
		//分割多个图像字串
		...
		if(!empty($info)){
			...
			$huifu = D('BudgetofferreplyView')->where($where)->order('id asc')->select();
			if(!empty($huifu)){
	...
	public function huifudata(){
		...
		$ids=M('budgetofferreply')->add($data);
		if($ids){
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/huifu/id/'.$id;
			//Header("Location:{$url}");  die;
			$this->redirect('Budgetoffer/huifu', array('id' => $weixiu_id ,'userid' => $rev_uid), 1, '回复成功...');die;
		}else{
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/addhuifu/id/'.$id.'/type/'.$type;
			//Header("Location:{$url}");  die;
			$this->redirect('Budgetoffer/huifu', array('id' => $weixiu_id , 'userid' => $rev_uid), 1,'回复失败...');die;
		}
