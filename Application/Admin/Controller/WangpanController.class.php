...
class WangpanController extends CommonController {
	...
	public function creat(){
		...
		$userlist = I('post.userlist',NULL);
		if(!empty($userlist) && ($userlist != 'null')){
	...
	public function uploadDocument(){
			...
			if(empty($nowid) || ($nowid == 0) || ($nowid == '0') || ($nowid == 'null')){
				$config = array(
						'path'         =>  WEB_DIR ."Public\\wangpan\\APP\\", //上传文件保存的路径
						'allowtype'       =>  array('txt','doc','pdf','zip','xls','ppt','docx','xlsx','pptx','rtf','wpx','docm','dotm','dotx','docx','rar','gz','zip','rar','gz'), //设置限制上传文件的类型
...
//上传笔记 'txt','doc','pdf','xls','ppt','docx','xlsx','pptx','php','rtf','wpx','docm','dotm','dotx','docx'
	...
	public function folderList(){
		...
		$where = array(
			...
			'_complex' => array(
				...
				'userlist' =>array(array('like',','.$userid.','),array('exp','is not null'),'AND'),
				'_logic'=>'or')
		);
		//当前进入的文件夹路径
	...
	public function filerename(){
		...
		if(empty($name)){
			...
			if(empty($info)){
				...
				if($pst === false){
					$dir = WEB_DIR .'Public\\wangpan\\APP\\'.$basedir.$nameEN;
					...
					if($ret){
						...
						if($xg){
							//原先文件夹名
							$oldname = WEB_DIR .'Public\\wangpan\\APP\\'.$basedir.$info['filename'];
							$newname = WEB_DIR .'Public\\wangpan\\APP\\'.$basedir.$nameEN;
					...
					$dir = WEB_DIR .'Public\\wangpan\\APP\\'.$basedir.$nameEN.$houzui;
					$ret = $this->checkname($dir);
					if($ret){
						$res = array(
							'code' => 202,
	...
	public function deletfile(){
		...
		foreach($arr as $key=>$id){
			...
			$dirName = WEB_DIR . 'Public\\wangpan\\APP\\'.$xiangduilujing;
	...
	public function filedownload(){
		...
		if(empty($info)){
			...
			if(($info['isdir'] === 1) || ($info['isdir'] === '1')){
				...
				$path = WEB_DIR .'Public\\wangpan\\APP\\'.$basedir.$info['filename'];
	...
	public function shareLink(){
		$id = I('post.id',0,'intval');
		//文件夹不可以分享
		$info = M('wangpan')->where(array('id'=>$id))->field('isdir,filename')->find();
		$arr = explode('.',$info);
		$filenamearr = $arr[0];
		$filename = $this->decodepath($filenamearr);
		$houzhui = strtolower($arr[1]);
		
		$tupian = array('jpg','png','gif','bmp','jpeg');
		$wendang = array('txt','doc','pdf','zip','xls','ppt','docx','xlsx','pptx','rtf','wpx','docm','dotm','dotx','docx','rar','gz');
		$shiping = array('mpg','mpeg','avi','rm','rmvb','mov','wmv','asf','dat','mp4','3gp','asx','mpe','wvx');
		$title = '';
		if(in_array($houzhui, $tupian)){
			$title = '图片分享';
		}else if(in_array($houzhui, $wendang)){
			$title = '文件分享';
		}else if(in_array($houzhui, $shiping)){
			$title = '视频分享';
		}else{
			$title = '分享';
		}
		$description = $filename.'.'.$houzhui;
		...
		if($info['isdir'] == 1){
			...
			$res = array(
				'code' => 200,
				'contentUrl' => 'http://59jiaju.com/manage/index.php/Admin/Wangpan/share/id/'.$id,
				'title' => $title,
	...
	public function share(){
		...
		if($id === 0){
			...
			if(in_array($houzhui, $tupian)){
				...
				$imgurl = 'Public\\wangpan\\APP\\'.$basedir.$info['filename'];
				$this->assign('imgurl',$imgurl);
			}else{
				//获取分享文件的内容
				$this->filedownload();
			}
			$nowdate = '当前日期：'.date('Y-m-d',time());
	...
	public function ttt(){
		$path = 'bG9nbw_e_e';
		//$one = $this->encodepath($path);
		$two = $this->decodepath($path);
		//$one = base64_encode($path);
