...
class HomedecorationController extends CommonController {
	//家装列表
	public function getList(){
		...
		$nowpage = $page;
		$userid = '';
		$data = M('Homedecoration')->alias('w')->join('ecm_gongyingshang g ON w.send_uid = g.user_id')->join('ecm_member m ON m.user_id = g.user_id')->field('w.*,g.name,m.user_name')->where($where)->order('w.id desc')->page($page,100)->select();
	    if($page == 0){
			$page = 1;
		}
		/*$where = array();
		...
		$rowcount = 0;
		 if(!empty($where)){ 
			$rowcount = D('HomedecorationView')->where($where)->count();
			//页码计算
			...
			$data = D('HomedecorationView')->where($where)->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('HomedecorationView')->where($where)->count();
		}else{
			$rowcount = D('HomedecorationView')->count();
			//页码计算
			$pagecount = ceil($rowcount/20);
			if(($page >= $pagecount) && ($pagecount != 0)){
				$page = $pagecount;
			...
			$data = D('HomedecorationView')->page($page,20)->order('id desc')->select();
			$count = count($data);
			$all_count = D('HomedecorationView')->count();
		}
		*/
	...
	public function setRevname(){
		...
		if($id && $mobile){
			if($mobile !== 'null'){
				$rev_name = M('homedecoration')->where(array('id'=>$id))->getField('rev_name',$rev_name);
				...
				M('homedecoration')->where(array('id'=>$id))->setField('rev_name',$rev_name);
				$res = array(
					'code' => 200,
					'message' => '设置成功！'
				);
	...
	public function getDetail(){
		...
		$id = I('get.id',0,'intval');
		$info = D('HomedecorationView')->where(array('id'=>$id))->find();
		if(!empty($info)){
			$rev_name = M('homedecoration')->where(array('id'=>$id))->getField('rev_name',$rev_name);
			$rev_name = trim($rev_name,',');
			$rev_name = explode(',',$rev_name);
			...
			//回复列表
			$data = D('HomedecorationreplyView')->where(array('jz_id'=>$info['id']))->order('id asc')->select();
			//按回话id分类回复
			$hh_idarr = D('HomedecorationreplyView')->distinct(true)->where(array('jz_id'=>$info['id']))->getField('hh_id',true);
			$mans = array();
	...
	public function huifu(){
		...
		//$id = 12;
		$info = M('homedecoration')->where(array('id'=>$id))->find();
		$res = array();
		//分割多个图像字串
		...
		if(!empty($info)){
			...
			$huifu = D('JiazhuangtransferView')->where($where)->order('id asc')->select();
			if(!empty($huifu)){
	...
	public function huifudata(){
		...
		$ids=M('jiazhuangtransfer')->add($data);
		if($ids){
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/huifu/id/'.$id;
			//Header("Location:{$url}");  die;
			$this->redirect('Homedecoration/huifu', array('id' => $weixiu_id ,'userid' => $rev_uid), 1, '回复成功...');die;
		}else{
			//$url = 'http://59jiaju.com/manage/index.php/Admin/Weixiumsg/addhuifu/id/'.$id.'/type/'.$type;
			//Header("Location:{$url}");  die;
			$this->redirect('Homedecoration/huifu', array('id' => $weixiu_id , 'userid' => $rev_uid), 1,'回复失败...');die;
		}
