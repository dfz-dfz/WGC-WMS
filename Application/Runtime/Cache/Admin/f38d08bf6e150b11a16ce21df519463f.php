...
<head>
<title>文档</title>
<link rel="stylesheet" href="/Public/admin/aui/BtFont/css/bootstrap.min.css">
<link rel="stylesheet" href="/Public/admin/aui/BtFont/css/font-awesome.min.css">
<!--弹窗-->
<style type="text/css"> 
.window,.window2{ 
    width:250px; 
...
<script type="text/javascript" src="/Public/admin/aui/script/jquery-1.7.2.js" ></script>
<!---------------------------------------------------------上传等待js-------------------------------------------------->
...
<script type="text/javascript" language="javascript">
    $(document).ready(function () {
		...
		$('#fileware').live('change',function(){ 
			...
			$.ajax({ 
				url : '/index.php/Admin/Wangpanpc/uploadDocument', 
				type : 'POST', 
				data : formData, 
				// 告诉jQuery不要去处理发送的数据
				processData : false, 
					...
					if(responseStr.code == 200){
						var wenjian = "<div class='laosu' style='min-width:80px;min-height:80px;width:10%;text-align:center;padding:5px;float:left;position:relative;'>";
							if((responseStr.isdir == 2)){
								wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h3.ico' style='width:100%;' />";
							}
...
<!----------------------------------------------------------------------弹窗开始--------------------------------------------------------------->
	...
	<div class="window" id="center"> 
		<div id="title" class="title"><img src="/Public/admin/aui/image/close.png" style='width:20px;' alt="关闭" />创建文件夹</div> 
		<div class="content">
	...
	<div class="window2" id="center2"> 
		<div id="title2" class="title2"><img src="/Public/admin/aui/image/close.png" style='width:20px;' alt="关闭" />重命名</div> 
		<div class="content2">
...
<div id='navi' style='width:100%;height:100%;padding:15px;overflow:auto;border:2px solid #C5C5C5;' >
	<div style='float:left;border:1px solid #C5C5C5;padding:5px;cursor:pointer;' id='download'>
		<i class="icon-download-alt"></i>下载
	</div>
	<div style='float:left;border:1px solid #C5C5C5;padding:5px;margin-left:20px;cursor:pointer;' id='btn_center2'>
		<i class="icon-edit"></i>重命名
	</div>
	<div style='float:left;border:1px solid #C5C5C5;padding:5px;margin-left:20px;cursor:pointer;' id='delete'>
		<i class="icon-trash"></i>删除
		<input type='hidden' id='ids' value='' />
	</div>
	<div style='float:left;border:1px solid #C5C5C5;padding:5px;margin-left:20px;cursor:pointer;' id="btn_center">
		<i class=" icon-folder-close-alt"></i>创建文件夹
	</div>
	<div style='float:left;border:1px solid #C5C5C5;padding:5px;margin-left:20px;cursor:pointer;' id="upload_file">
		<i class="icon-upload-alt"></i>上传文件
	</div>
	<form id='documentuploads' action='/index.php/Admin/Wangpanpc/uploadDocument' method='post' enctype="multipart/form-data" >
...
<div>
	...
	<input type='hidden' id='puserid' value='<?php echo ($_SESSION['wgcadmininfo']['user_id']); ?>' />
	<input type='hidden' id='ptypes' value='2' />
...
$(document).ready(function(){
	...
	//ajax获取首层文档文件夹列表
	$.post("/index.php/Admin/Wangpanpc/folderList", {  "pid": ppid+"", "userid": puserid+"","types": ptypes+"","levels": plevels+"","display":pdisplay+"","isdir":isdir+""},
	   function(data){
		 if(data.code == 200){
			...
			for(var o in msg){ 
				...
				wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h6.png' style='width:100%;' />";
				}
				if((msg[o].types==3) && (msg[o].isdir==1)){
				wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h12.png' style='width:100%;' />";
				}
				if((msg[o].types==4) && (msg[o].isdir==1)){
				wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h2.png' style='width:100%;' />";
				}
				if((msg[o].types==5) && (msg[o].isdir==1)){
				wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h13.png' style='width:100%;' />";
				}
				if(msg[o].isdir == 2){
					wenjian += "<img class='enterimg' src='/Public/admin/aui/image/6505/h3.ico' style='width:100%;' />";
				}
	...
	$("#download").bind("click", function(){
		...
		if(length <= 0){
			...
			if(isdir == 1){
				...
				//下载文件
				var url = "/index.php/Admin/Wangpanpc/filedownload/id/"+pid+"";
				//alert(url);
	...
	$("#create").bind("click", function(){
		...
		if(wenjianming == ''){
			...
			if($('#plevels').val() >=4){
				layer.msg('文件夹只能创建四层',{icon: 5,time:1000});
			}else{
				//指示牌取值
				var ppid = $('#ppid').val();
				var puserid = $('#puserid').val();
				...
				var pdisplay = $('#pdisplay').val();
				//上传创建文件数据创建成功后添加 
				$.post("/index.php/Admin/Wangpanpc/creat", { "pid": ppid+"", "userid": puserid+"","filename": wenjianming+"","types": ptypes+"","levels": plevels+"","display":pdisplay+""},
				   function(data){
					 if(data.code == 200){
						var wenjian = "<div class='laosu' style='min-width:80px;min-height:80px;width:10%;text-align:center;padding:5px;float:left;position:relative;'>";
		...
		if(length <= 0){
			...
			if(newname == ''){
				...
				if(isdir == 1){
					...
					//ajax修改文件（夹）名
					$.post("/index.php/Admin/Wangpanpc/filerename", { "id": pid+"","userid": puserid+"", "filename": newname+""},
					   function(data){
	...
	$("#delete").bind("click", function(){
		...
		if(length <= 0){
			...
			$.post("/index.php/Admin/Wangpanpc/deletfile", { "ids": ids+"","userid": puserid+""},
			   function(data){
				  var message = data.message;
				 if(data.code == 200){
					$('#ids').val("");
	...
	$('.enterimg').live('click', function(){
		...
		if(levels < 6){
			...
			//ajax获取第二层文档文件夹列表
			$.post("/index.php/Admin/Wangpanpc/folderList", { "pid": pid+"", "userid": userid+"","types": types+"","levels": levels+"","display":display+"","isdir":isdir+""},
			   function(data){
				 if(data.code == 200){
					//清空列表
					$("#mainbody").empty();
					...
					for(var o in msg){ 
						...
						if((msg[o].types==2) && (msg[o].isdir==1)){
						wenjian2 += "<img class='enterimg' src='/Public/admin/aui/image/6505/h6.png' style='width:100%;' />";
						}
						if((msg[o].types==3) && (msg[o].isdir==1)){
						wenjian2 += "<img class='enterimg' src='/Public/admin/aui/image/6505/h12.png' style='width:100%;' />";
						}
						if((msg[o].types==4) && (msg[o].isdir==1)){
						wenjian2 += "<img class='enterimg' src='/Public/admin/aui/image/6505/h2.png' style='width:100%;' />";
						}
						if((msg[o].types==5) && (msg[o].isdir==1)){
						wenjian2 += "<img class='enterimg' src='/Public/admin/aui/image/6505/h13.png' style='width:100%;' />";
						}
						if(msg[o].isdir == 2){
							wenjian2 += "<img class='enterimg' src='/Public/admin/aui/image/6505/h3.ico' style='width:100%;' />";
						}
